...
нет и нет passwordless sudo, чтобы их поставить — зато есть powershell.exe,
доступный из WSL по абсолютному пути, и он говорит напрямую с реальным аудио
Windows-хоста. Воспроизведение — через System.Media.SoundPlayer, запись —
через MCI (mciSendString).

SoundPlayer, которому дали путь через UNC \\wsl.localhost\\... (так же как
WSL монтирует Windows-диски), либо не играет вообще, либо завершается за
миллисекунды вместо реальной длительности файла — без единой ошибки. Тот же
файл, скопированный в C:\\Windows\\Temp (обычный локальный Windows-путь),
проигрывается корректно. Поэтому здесь ВСЕГДА работаем через файлы под
_WIN_TMP (который у WSL примонтирован как /mnt/c/Windows/Temp) — никогда
напрямую по WSL-путям.
"""
import os
import queue
import subprocess
...
# .venv, где стоит diffusers/SDXL (см. tts_worker.py про этот конфликт).
# Поэтому синтез — всегда subprocess, никогда прямой import.
...
def record_from_mic(seconds: int) -> str | None:
    ...
    ресемплинга и артефактов, которые давала 8-битная запись). Возвращает
    WSL-путь к WAV или None при сбое PowerShell-моста."""
...
def synthesize_speech(text: str) -> str | None:
    ...
    звать через run_in_executor из async-кода (RTF ~4-5 на этом CPU — это не
    быстрая операция). Клонирует голос
...
class SpeechStreamer:
    ...
    на весь текст. RTF ~4-5 на этом CPU (см. synthesize_speech) — ждать,
    пока модель допишет ВЕСЬ ответ, и только потом синтезировать его
    целиком означало бы полную сумму (время генерации + время синтеза)
