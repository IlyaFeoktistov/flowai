...
# (команда, аргументы, описание) — Table.grid делает то, что раньше
# пытались руками добить пробелами прямо в тексте (см. git-историю):
# выравнивает колонки и, что руками невозможно, корректно переносит
# длинное описание на следующую строку С СОХРАНЕНИЕМ отступа под своей
# же колонкой вместо наезда на соседние или обрезания по ширине терминала.
_HELP_ROWS: list[tuple[str, str, str]] = [
    ("/gen", "промпт", "сгенерировать картинку напрямую"),
    ("/img", "путь.jpg", "картинка с диска → [Image-N]"),
    ("/paste", "", "картинка из буфера → [Image-N]"),
    ("/music", "промпт", "потоковая музыка, /music ещё раз или Ctrl+C — стоп"),
    ("/music_gen", "промпт", "один трек напрямую, без агента"),
    (
        "/gen_model", "промпт --rig --raw --lod N",
        "3D-модель; --rig — со скелетом, --raw — без ретопологии, --lod N — ещё N моделей "
        "пониже полигонажем (_lod1.._lodN); @имя.png — картинка из img-refs/, несколько @ — "
        "батч (по модели на картинку, не одна общая)",
    ),
    (
        "/gen_model", "--front @имя ...",
        "ОДНА модель из нескольких ракурсов ОДНОГО объекта (Hunyuan3D-2mv): front — прямо на "
        "объект (0°), left — поворот на 90° влево, back — со спины (180°), right — поворот на "
        "90° вправо. Нужен минимум front, для реального эффекта — front+left+back. Если "
        "референсы уже названы front.png / left.png / back.png — можно просто "
        "/gen_model @front @left @back, без флагов (определяется по именам)",
    ),
    ("/anim", "описание движения", "оживить последнюю риггованную модель, или /anim @имя ... — конкретную (Animato)"),
    ("/gen_texture", "@модель @картинка", "перегенерировать текстуру готовой модели по референсу, порядок аргументов не важен"),
    ...
    (
        ...
        "с аргументами — только эти файлы / папки (не трогая остальной уже собранный индекс)",
    ),
    (
        "/clean", "logs | trash | snapshots | projects | all",
        "почистить накопившийся за всё время хлам (логи / корзину / индексы); без аргумента — "
        "только отчёт, ничего не удаляет",
    ),
    ("/update", "", "проверить и подтянуть обновления flowAI из git"),
    ("/settings", "", "настройки моделей и GPU"),
...
def _show_help() -> None:
    ...
    table.add_column(style="dim", max_width=34)
    table.add_column()
...
async def main() -> None:
    ...
    try:
        ...
        # expert-streaming (порт 8090) намеренно НЕ останавливается при
        # выходе — модель остаётся загруженной, следующий запуск подхватывает
        # её через expert_streaming._STATE_PATH вместо минутной перезагрузки.
        # Выгрузить вручную — /instances.
