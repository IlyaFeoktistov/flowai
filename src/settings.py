...
_state: dict = {
    ...
    # путь основного агента на реальных задачах уже наглядно ломался (план текстом
    ...
    # (уходит в бесконечный повтор/слоп без верификации ответа — см. коммит
    # "Fix casual-chat coherence collapse"), выключить тут — тогда те же
    # сообщения (needs_change=false) идут в обычную
    ...
    # ответах, чем голый answer_casual. Не действует на основной агент (у
    ...
    # tools=[] путь). Дефолт ВЫКЛ — не меняет поведение основного агента, пока
    ...
    # pipeline_mode — основной агент и роли пайплайна строят модель через один
    # и тот же agent_builder._build_chat_model.
    "expert_streaming_enabled": os.getenv("EXPERT_STREAMING_ENABLED", "1") == "1",
    # Нужен как отдельный, редактируемый тумблер, а не только
    # mcp_agent/model_config.py:OLLAMA_NUM_CTX (тот
    ...
    # разделением, а багом: если num_ctx занижен относительно
    # OLLAMA_NUM_CTX=65536 (например до 16384 для тестирования), порог
    # компакта по-прежнему считался от OLLAMA_NUM_CTX, так что компакт ждал
    # вдвое больше токенов, чем реально помещалось в модель, и ни разу не
    # срабатывал за весь ход. compaction.py теперь тоже читает
