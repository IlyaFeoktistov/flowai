...
# ход, не содержательный ответ по теме заданного вопроса. Без этой
# проверки такой ответ (например "остановись" на бессмысленный
# уточняющий вопрос Planner'а) уходит дальше как ОБЫЧНЫЙ текст в guidance
# для следующей попытки той же стадии — self-heal исправно продолжает
# ретраить (Planner обязан вызвать ask_user перед завершением), полностью
# игнорируя то, что пользователь просил прекратить, пока Planner не
# дойдёт до max_attempts, так и не остановившись. Точное совпадение ПОСЛЕ normalize,
