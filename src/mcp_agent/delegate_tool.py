...
Живой прогон (mail-server, 20260707-185843-99d14cf5, см. разбор в истории
чата) показал реальную цену того, что ВСЁ расследование идёт в ОДНОМ общем
контексте с ОДНИМ общим бюджетом шагов (RECURSION_LIMIT): агент честно
копал цепочку Job -> Service -> Repository -> Persister -> UnitStarter и
кончился шагами, похоже, в файле от готового ответа.
...
# Живой тест ЭТОГО тула (build_delegate_tool + qwen3-coder:30b, без единого
# приближения): с 25 read-only тулами модель НЕ сформировала настоящий
# structured tool_calls, а слила его текстом в content
# ("<function=read_text_file><parameter=path>...") — тот же баг, из-за
...
# Живой прогон (mail-server, реальная задача, 37 минут, 106 вызовов тулов,
# 2 258 084 входных токена, 0 правок): промпт дважды явно советовал звать
# delegate для ровно такого расследования — модель НИ РАЗУ его не вызвала ни
# в одной из 3 попыток, просто читала файлы вручную до потери всякого
# бюджета. Совет текстом в системном промпте на длинном ходу не работает —
# нужна детерминированная проверка, а не ещё одна просьба.
#
...
# единственный такой сейчас. Живой прогон: пока delegate работал (~5 минут
# на реальной задаче), внешний agent.astream(stream_mode=["values",
# "messages"]) выдавал ДЕСЯТКИ answer_start подряд без единого answer_chunk/
# answer_end — похоже, токен-стрим вложенного sub_agent просачивается в
...
async def _run_subagent_streaming(sub_agent, conversation: list, config: dict) -> dict:
    ...
    (тот canal стриминга ДРУГОЙ, см. stage_runner.py:_SUBAGENT_TOOLS про
    живой баг с утечкой чужих answer_start в внешний stream_mode=
    ["values","messages"] при *любом* вызове сабагента на том же shared
    model — использование astream здесь для ЗНАЧЕНИЙ графа, не для токенов,
    не трогает эту утечку сильнее, чем раньше делал ainvoke."""
