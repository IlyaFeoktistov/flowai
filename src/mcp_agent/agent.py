...
from mcp_agent.message_utils import _calls_by_id, _to_lc_messages, _tool_artifact_diagnostics, _tool_artifact_diff, _tool_text  # noqa: E402
from mcp_agent.model_config import DEBUG, MAX_ATTEMPTS, RECURSION_LIMIT, TOOL_OUTPUT_CHAR_CAP  # noqa: E402
            ...
            async for mode, payload in agent.astream(
                ...
                for m in new_msgs:
                    if isinstance(m, AIMessage):
                        ...
                        # Match the model's own output cap instead of a flat
                        # 2000 — that used to cut mid-line/mid-path before
                        # ui/stream.py's own tool_end renderer (fold, or the
                        # line-based preview + "N more lines" marker) ever
                        # saw the result.
                        log_event("tool_result", name=m.name, result=result_text[:TOOL_OUTPUT_CHAR_CAP])
                        if on_event:
                            ...
                            event = {"type": "tool_end", "name": m.name, "result": result_text[:TOOL_OUTPUT_CHAR_CAP], "id": m.tool_call_id}
                            if diff is not None:
                                event["diff"] = diff
...
async def stream_chat(messages: list[dict], on_event=None, mid_turn_queue=None) -> Any:
    ...
    if final_verdict is not None and not final_verdict["relevant"]:
        ...
        # никакого ретрая всё равно не будет. Этот баннер раньше вешался на
        # ЛЮБОЙ отклонённый verdict, включая ложные срезы judge'а (например
        # когда судья цепляется за стейл-контекст уже отброшенного
        # промежуточного tool call), и выглядело как "модель всегда отвечает
        # неправильно", хотя
    ...
    # update_knowledge (даже при явной инструкции в system prompt модель
    # ненадёжно вспоминает об этом тулe). Порог ≥4 разных мест разведки —
