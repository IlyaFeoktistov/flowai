...
def _build_system_prompt(repo_path: str) -> str:
    ...
    # generate_image/edit_image/generate_music tools don't exist for this
    # session at all when gen_agent_tools is off (config.py:build_mcp_
    # connections pops their MCP servers entirely, not just their tool
    # names downstream) — describing them anyway would waste prompt tokens
    # on capabilities the model literally cannot call, every single turn.
    # analyze_image stays in the base template unconditionally: its "vision"
    # server is never gated by this setting (see config.py's own comment).
    gen_tools_block = (
        "- generate_image(prompt, negative_prompt) / edit_image(path, "
        "prompt, negative_prompt) — generate_image creates a NEW picture "
        "from a text description only; edit_image MODIFIES an existing one "
        "(restyle it, change objects/colors/setting) while keeping its "
        "composition — never for 'write'/'generate' CODE despite sharing "
        "the verb. `prompt` MUST be in English only for both. edit_image's "
        "amount of change is a user-configured setting, not a call "
        "argument — don't invent a 'strength' parameter. A pasted/attached "
        "image's real path (see analyze_image above) goes to edit_image, "
        "never to generate_image.\n"
        "- generate_music(prompt, duration) — generate INSTRUMENTAL music/"
        "sound from a text description (no vocals/lyrics). `prompt` MUST "
        "be in English only. CPU-only and noticeably slower than speech/"
        "image generation — warn the user it'll take a while.\n"
        if settings.get("gen_agent_tools") else ""
    )
    prompt = _SYSTEM_PROMPT_TEMPLATE.format(
        repo_path=repo_path, env_block=env_block, delegate_override=delegate_override,
        gen_tools_block=gen_tools_block,
    )
...
_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "alone — call `git show <commit>` to see what it "
    ...
    "safety check, not second-guessing here. bash has no TTY attached — a "
    "command needing sudo or any other interactive credential/confirmation "
    "prompt will hang or fail. Don't retry it or guess/pipe in a password — "
    "tell the user to run the exact command themselves via the CLI's "
    "'!<command>' prefix (their real TTY/credentials, output comes back as "
    "their next message), and stop there.\n"
    ...
    "never for 'write'/'generate'/'create' CODE even though it shares a "
    "verb with generation tools. If the user pasted/attached an image (via "
    "clipboard paste or /img), their message contains a real, "
    "already-existing file path (usually under a system temp "
    "'flowai-uploads' directory) instead of a picture — pass that exact "
    "path here, never guess/invent one.\n"
    "{gen_tools_block}"
    ...
    "you're about to do — then call the tool(s) in the same turn. No long "
    ...
    "  - If you need several independent pieces of information (reading "
    "multiple files, unrelated searches, `git show` on several commits), "
    "issue ALL of them together as multiple tool calls in the same turn, "
    "not one at a time — each one-at-a-time round trip costs a full extra "
    "model turn. Only go one at a time when a later call's arguments "
...
_OPTIMIZED_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "nothing to show for it. Start narrow "
...
def _analyzer_system_prompt(env_block: str) -> str:
    return (
        ...
        "(query it directly instead of searching the filesystem for its "
        "underlying files), a runtime's actual version/output (`node "
        "--version`, `php -v`), or running something to see its real "
        "behavior. Keep the command narrow and SPECIFIC to the one exact "
        "thing you need, never a broad filesystem sweep. If a command "
