...
_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "when).\n"
    "- A question about flowAI ITSELF (what it can do, how it works) or a "
    "request to write/add an extension for yourself — a skill, a custom "
    "sub-agent, a slash command, a hook, a plugin: call flowai_guide "
    "FIRST. It gives the exact folders and file formats; don't search the "
    "disk for examples or guess a plausible-sounding format.\n"
    "{delegate_override}\n"
    ...
    "    If the result includes a \"Found N new diagnostic issue(s)\" "
    "block, that's a real error/warning from the project's actual language "
    "server, introduced by the edit you just made — read it and fix it "
    "with another edit_file call before moving on, don't just note it and "
    "continue as if the edit succeeded cleanly.\n"
...
_OPTIMIZED_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "case), write_file only for a new file or a near-total rewrite. If the "
...
def _analyzer_system_prompt(env_block: str) -> str:
    return (
        ...
        f"{env_block}\n\n"
        "If the request is to write/add a skill, custom sub-agent, slash "
        "command, hook or plugin for flowAI itself (not something the "
        "currently open project's own code does) — call flowai_guide "
        "FIRST for the exact folders and file formats instead of "
        "searching the disk for examples or guessing.\n\n"
        "When you need several INDEPENDENT pieces of information — several "
...
def _coder_system_prompt(env_block: str) -> str:
    return (
        ...
        "If write_file/edit_file's result includes a \"Found N new "
        "diagnostic issue(s)\" block, that's a real error/warning from the "
        "project's actual language server, introduced by the edit you just "
        "made — read it and fix it with another edit_file call before "
        "moving on to the next step, don't just note it and continue as if "
        "the edit succeeded cleanly.\n\n"
...
def _quick_fix_system_prompt(env_block: str) -> str:
    return (
        ...
        "write_file for one spanning most of a file or a brand-new one. If "
        "the result includes a \"Found N new diagnostic issue(s)\" block, "
        "that's a real error your edit just introduced — fix it with "
        "another edit_file call right away, don't leave it for the check "
        "below to catch.\n\n"
