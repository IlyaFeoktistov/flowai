...
- `agent`/`agent_result` (`mcp_agent/delegate_tool.py`, см. «Агенты» ниже), `web_read`
...
основной агент и его сабагенты — везде, где уже есть `_WEB_TOOLS`/`fetch`.
...
## Агенты (сабагенты, как в Claude Code)

`agent(description, prompt, subagent_type="explore", run_in_background=false)`
— основной агент (не агентный режим) запускает сабагента со своим
контекстом и бюджетом шагов. Типы (`mcp_agent/subagents.py`):

| Тип | Что может |
|---|---|
| `explore` | только чтение: разобраться, как устроено, где определено, что вызывает что — отчёт с file:line |
| `plan` | только чтение: план реализации изменения |
| `general` | все тулы основного агента, включая правки файлов и bash — делает кусок работы целиком и отчитывается |

Свои агенты — `.flowai/agents/<имя>.md` в проекте или `~/.flowai/agents/<имя>.md`:
frontmatter `name`, `description` (когда его звать), `tools` (список; без него —
все тулы), тело — системный промпт агента. Проектный перекрывает пользовательский
и встроенный с тем же именем. Список типов с описаниями — в системном промпте.

- **Одна модель, без второго инстанса.** Все агенты работают на той же
  загруженной модели. Сколько генерируют одновременно — настройка
  «параллельные потоки» (`parallel_slots`, дефолт 1, максимум 8) = слоты
  llama-server (`-np`); `-c` = `num_ctx × потоки`, то есть каждый поток
  сверх первого — ещё один KV-cache окна `num_ctx` в памяти (не веса), а
  скорость генерации делится между запросами. При 1 агенты идут строго по
  очереди. На Ollama-пути параллельность задаёт демон (`OLLAMA_NUM_PARALLEL`).
- **Фоновый запуск.** `run_in_background=true` возвращает `agent_id` сразу —
  основной агент продолжает работу, отчёт забирает `agent_result(agent_id)`.
  При одном потоке фоновый агент ждёт своей очереди к модели. Ход не
  заканчивается на несобранных фоновых агентах: если модель дала финальный
  ответ, не забрав их, `stage_runner.run_stage` ждёт их отчёты (в UI — вызов
  «жду фоновых агентов»), отдаёт модели и ход продолжается; при остановке
  хода оставшиеся фоновые агенты отменяются.
- **Пишущие агенты** используют те же обёртки тулов, что основной агент
  (снимок перед записью, проверка свежести чтения), и спрашивают разрешения
  тем же диалогом (CLI и веб); запись вне проекта и хуки плагинов — как у
  основного. В plan-режиме пишущие типы запрещены.
- Сабагент не видит разговор и не может спрашивать пользователя — `prompt`
  должен быть самодостаточной задачей; вложенных агентов нет.
