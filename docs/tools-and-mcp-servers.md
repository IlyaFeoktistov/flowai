...
| `lsp` | `lsp` | настоящий Language Server Protocol (goToDefinition/findReferences/hover/...) вместо grep-угадайки. Клиентская машинерия (запуск/маппинг серверов по расширению, пул по (команда, корень репозитория)) вынесена в общий `_lsp_client.py` — его же использует `file_ops` для diagnostics-после-правки выше; `lsp_server.py` и `file_ops_server.py` — два разных процесса, каждый со своим пулом запущенных языковых серверов |
...
основной агент и `delegate` — везде, где уже есть `_WEB_TOOLS`/`fetch`.
...
тул нужен в новом пайплайне, не только основному агенту) добавить его имя в
