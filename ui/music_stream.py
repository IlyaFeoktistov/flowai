...
наперёд, пока играет текущий — тот самый "склад в очередь", о котором шла
речь. На этом CPU генерация куска обычно МЕДЛЕННЕЕ его собственной
длительности (см. живые замеры generate_music в этой сессии), так что реально
догнать playback и не отставать редко получается — очередь просто
естественно пустеет, и плеер ждёт на queue.get() между кусками. Это не
гарантия gapless-потока, а честная попытка спрятать ожидание там, где для
этого хватает запаса по CPU.
...
async def stream_music(prompt: str, on_status=None) -> None:
    ...
    try:
        ...
        _stop_flag = True
        gen_task.cancel()
        # Живой баг: cancel() без await не значит "поток встал" — если
        # генератор в этот момент сидел внутри run_in_executor
        # (generate_clip_sync крутится на CPU/GPU в отдельном треде),
        # отмена asyncio-таска НЕ прерывает сам тред: тяжёлое вычисление
        # доигрывает до конца само, просто его результат потом отбрасывается.
        # Без этого await stream_music() (и созданный от него _music_task в
        # cli.py) считался "завершённым" мгновенно — /music, вызванный
        # заново сразу после стопа, стартовал ВТОРУЮ генерацию, пока первая
        # ещё молотила в фоне тем же CPU/GPU. await здесь заставляет нас
        # реально дождаться, пока тред отработает и CancelledError долетит.
        try:
            await gen_task
