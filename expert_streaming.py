...
def ensure_running(
    ...
    cmd = [
        ...
        # expert cache is OFF" и просто грузит модель как обычно, без
        # кэша экспертов вообще — вся затея молча превращается в no-op.
        # -fit (авто-подбор -ngl под свободную VRAM) включён по умолчанию
        # (common/common.h: fit_params=true) и обязан остаться единственным,
        # кто решает -ngl, иначе -ehs autofit не может совместно подобрать
        # ngl И число горячих слотов.
        "-ehs", "-1",
        "--expert-sidecar",
        "--host", DEFAULT_HOST,
        "--port", str(port),
