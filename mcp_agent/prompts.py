...
_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "expected_first_line/expected_last_line from read_file_range or "
    "search_code's numbered output ONLY — never by eyeballing/counting a "
    "raw read_file/read_text_file dump, which shows no line numbers at all "
    "and is exactly how a range ends up off by one or two lines. "
    "expected_first_line/expected_last_line are each exactly ONE line (the "
    "line AT start_line/end_line) — never the whole old block you're "
    "replacing. ALWAYS pass "
    "expected_first_line/expected_last_line (the exact text you last saw "
    "there, verbatim) — not just when unsure about the range, every single "
    "call. Live bug: a replace_lines miscounted end_line by one and "
    "silently deleted a method's own signature line ('public function "
    ...
    "file with write_file. expected_first_line/expected_last_line would "
    ...
    "insert next to it is exactly the kind of byte-for-byte mistake this "
    "tool exists to avoid. Pass expected_line the same way as expected_"
    "first_line above.\n"
    ...
    "before your next edit to it, or rely on expected_first_line/"
    "expected_last_line/expected_line to catch a wrong guess instead of "
    ...
    "get wrong.\n"
    "- ALWAYS pass expected_first_line/expected_last_line to replace_lines "
...
_OPTIMIZED_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "expected_first_line/expected_last_line (the exact text of those two "
    "lines as you last saw them, from read_file_range/search_code's "
    "numbered output, never eyeballed) — this catches a stale range (file "
    "changed, or an earlier edit this turn shifted line numbers) with a "
    "loud error instead of a silent wrong edit. Pass new_content='' to "
    "delete the range.\n"
    ...
    "to splice code next to it. Pass expected_line the same way as "
    "replace_lines' expected_first_line. Do NOT reconstruct new_content "
...
def _coder_system_prompt(env_block: str) -> str:
    return (
        ...
        "block. Get start_line/end_line AND expected_first_line/"
        "expected_last_line from read_file_range or search_code's numbered "
        "output ONLY — never by eyeballing/counting a raw read_file/"
        "read_text_file dump, which has no line numbers at all. "
        "expected_first_line/expected_last_line are each exactly ONE line "
        "(the line AT start_line/end_line) — not the whole old block. "
        "ALWAYS pass both — every single call, not "
        "just when unsure. Live bug: a replace_lines miscounted end_line by "
        "one and silently deleted a method's own signature line instead of "
        "replacing what was intended — expected_first_line/"
        "expected_last_line would have caught the miscount immediately (a "
        "loud mismatch error) instead of a silent, wrong edit discovered "
        "only later.\n"
        "  * insert_lines(path, line, new_content) — PREFERRED whenever "
        ...
        "you don't actually need; line=0 also needs no expected_line at "
...
def _quick_fix_system_prompt(env_block: str) -> str:
    return (
        ...
        "a raw read_file/read_text_file dump (no line numbers shown), and "
        "expected_first_line/expected_last_line are each exactly ONE line "
        "(the line AT start_line/end_line), never the whole old block. "
        "ALWAYS pass both — it catches a miscounted range immediately "
        "instead of a silent wrong edit.\n"
