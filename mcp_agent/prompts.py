...
_OPTIMIZED_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "'path' argument of read_file_range/write_file/edit_file — there, "
    ...
    "everything, or creates a new one). Use it for a brand-new file, or a "
    "change spanning most of an existing one. Never use it to change one "
    "line of a large existing file — that means retyping the whole file "
    "from memory, which is exactly how a byte silently changes something "
    "that didn't need to; use edit_file for a targeted change instead.\n"
    "- edit_file(path, edits) — makes one or more targeted find/replace "
    "edits in an EXISTING file. `edits` MUST be a JSON ARRAY of "
    "{{oldText, newText}} objects, even for a single change — never a bare "
    "object. Example: edit_file(path=\"src/app.py\", "
    "edits=[{{\"oldText\": \"DEBUG = False\", \"newText\": \"DEBUG = True\"}}]). "
    "oldText must match the file byte-for-byte (get it from a real "
    "read_file_range/search_code result you just saw, never retyped from "
    "memory) — the shorter and more exact oldText is, the less likely it "
    "is to fail to match.\n\n"
    ...
    "  2. Write the change — write_file for a new file or a near-total "
    "rewrite, edit_file for a targeted change to an existing one.\n"
