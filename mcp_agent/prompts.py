...
def _detect_system_tools() -> str:
    ...
    методом проб и ошибок через bash (`python` -> not found, `python3`
...
def _analyzer_system_prompt(env_block: str) -> str:
    return (
        ...
        "sentence. ssh/scp to a remote host, or any shared/dev/staging/"
...
def _coder_system_prompt(env_block: str) -> str:
    return (
        ...
        "do NOT have bash — verifying the change is the Verifier "
        ...
        "from THIS session, keeping earlier ones — you have no bash, so "
        "this is your only revert path (git-based revert to an arbitrary "
        "commit needs bash, which only Analyzer/Planner/Verifier have). "
...
def _quick_fix_system_prompt(env_block: str) -> str:
    return (
        ...
        "(status/diff/log/show) is not available here — quick-fix has no "
        "bash.\n\n"
        ...
        "You do NOT have bash — verifying the change is the Verifier "
