"""mcp_agent/stages/main_agent.py — the verdict/guidance tree extracted from
...
from mcp_agent.stages.main_agent import main_guidance, make_main_verdict
...
async def test_no_tools_no_question_is_relevant_without_calling_the_judge(judge_stub):
    ...
    verdict_fn = make_main_verdict("JUDGE", "do X", on_event=None)
...
async def test_earlier_failed_write_retried_and_fixed_later_in_the_round_reaches_the_judge(judge_stub):
    ...
    verdict_fn = make_main_verdict("JUDGE", "fix the bug", on_event=None)
...
async def test_verifying_events_wrap_every_judge_call(judge_stub):
    ...
    verdict_fn = make_main_verdict("JUDGE", "do X", on_event)
...
def test_guidance_no_tools_branch_does_not_repeat_the_reason():
    ...
    g = main_guidance({"relevant": False, "reason": "x"}, [], [], "what now?")
...
def test_guidance_execution_failure_includes_the_real_bash_error():
    ...
    new_tool_msgs = [m for m in round_msgs if hasattr(m, "tool_call_id")]
    g = main_guidance({"relevant": False, "kind": "execution_failure", "reason": "r"}, round_msgs, new_tool_msgs, "")
...
def test_guidance_falls_back_to_generic_advice_when_no_specific_condition_matches():
    ...
    new_tool_msgs = [m for m in round_msgs if hasattr(m, "tool_call_id")]
    g = main_guidance({"relevant": False, "reason": "off topic"}, round_msgs, new_tool_msgs, "some answer")
