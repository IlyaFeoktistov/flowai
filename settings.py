...
_state: dict = {
    ...
    # compaction.py по-прежнему используют неизменный
    # model_config.OLLAMA_NUM_CTX (см. их собственные импорты) — намеренное,
    # узкое разделение, а не забытое место: те не имеют своей копии этого
    # тумблера и не участвуют в том, что тут тестируется. Понижение сильно
    # ниже дефолта — это ОСОЗНАННЫЙ риск возврата бага "n_keep=4, промпт
    # обрезается, модель забывает задачу" (см. подробный разбор в
    # model_config.py прямо над OLLAMA_NUM_CTX) — 65536 там был не
    # произвольным числом, а посчитанным потолком под 24 GB RAM этой машины,
    # так что при использовании (не только тестировании!) меньшего значения
    # эффективный TOOL_OUTPUT_CHAR_CAP/длина истории должны соответствовать
    # новому потолку, тумблер сам этого не гарантирует.
    ...
    # обратный импорт был бы циклическим, см. комментарий там же про
    # COMPACT_HISTORY_CTX_RATIO). Расхождение не ломает ничего катастрофично
    ...
    # реально приближается к num_ctx (см. COMPACT_HISTORY_CTX_RATIO в
    # model_config.py). Дефолт ВКЛ — без этого длинный тул-цикл рискует
